...
public class BlurCommand extends Command {
  public BlurCommand(IModel model, IView view) {
    super(model, view);
  }
  ...
  protected void execute(String[] args) {
    if (args.length == 5) {
      ...
      callViewSuccess("Preview Blur operation executed successfully.");
    }
    if (args.length == 3) {
      String name = args[1];
      String destImageName = args[2];

      model.blurImage(name, destImageName);
      callViewSuccess("Blur operation executed successfully.");
    }
  }
