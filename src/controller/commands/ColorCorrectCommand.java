...
/** This class represents a color correct command. */
public class ColorCorrectCommand extends Command {
  public ColorCorrectCommand(IModel model, IView view) {
    super(model, view);
  }
  ...
  protected void execute(String[] args) throws IOException {
    if (args.length == 5) {
      ...
      callViewSuccess("Preview Color Correction operation executed successfully.");
    }
    if (args.length == 3) {
      String name = args[1];
      String destImageName = args[2];

      model.colorCorrect(name, destImageName);
      callViewSuccess("Color Correction executed successfully.");
    }
  }
