...
public class SepiaCommand extends Command {
  public SepiaCommand(IModel model, IView view) {
    super(model, view);
  }
  ...
  protected void execute(String[] args) {
    if (args.length == 5) {
      ...
      callViewSuccess("Preview Sepia operation executed successfully.");
    }
    if (args.length == 3) {
      String name = args[1];
      String destImageName = args[2];

      model.sepiaImage(name, destImageName);
      callViewSuccess("Sepia operation executed successfully.");
    }
  }
